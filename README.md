...
- 상품 탐색: 카테고리/목록/상세 조회, 조회수(hit) 반영
- 회원 기능: 회원가입/로그인, 마이페이지(주문내역/정보 확인)
- 장바구니: 상품 담기/수량 변경/삭제
- 회원은 id, 비회원은 session 기반으로 저장
- 주문/결제 플로우: 주문 생성 → 주문번호(ordernum) 발급 → 배송정보 입력/저장
- 후기(게시판): 구매한 상품에 대해 후기 작성(별점 star), 상품코드/주문번호 연동

### 관리자(Admin)

- 상품 관리: 상품 등록/수정/삭제, 상품 리스트 관리
- 회원 관리: 회원 목록 조회, 승인/등급(approved/class) 관리
- 주문 관리: 주문 내역 조회, 배송 상태(status) 확인
