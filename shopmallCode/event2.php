...
<tr><td ><font style='font-size:12px;font-weight:bold;'> Home > Promotion > 물고기 아이템 컬렉션</font></td>
	</tr></table>
<table border=0  style='margin-top:-5px;width:100%'>
<tr><td style='width:100%' align=center  ><img src='event2222.jpg'  ></td>
	</tr>	
<tr><td style='width:100%' align=center  ><img src='event22.jpg'  ></td>
	</tr>
	<tr><td style='width:100%' align=center  ><img src='event2.jpg'  ></td>
	</tr>
	<tr><td style='width:100%' align=center  ><img src='event222.jpg'  ></td>
	</tr>
	");
	$con = mysql_connect("localhost","root","apmsetup");
	mysql_select_db("shopmall",$con);
	$result=mysql_query("select * from product where name like '%물고기%'",$con);

//if (!isset($class)) $class=0;

		switch($mode) {
   case 0:        // 초기화면에 출력할 인기 상품 목록
       $result = mysql_query("select * from product where name like '%물고기%' order by name", $con);
		break;
   case 1:        // 초기화면에 출력할 인기 상품 목록
       $result = mysql_query("select * from product where name like '%물고기%' order by price2", $con);
		break;
	case 2:        // 초기화면에 출력할 인기 상품 목록
       $result = mysql_query("select * from product where name like '%물고기%' order by price2 desc", $con);
		break;
	case 3:        // 초기화면에 출력할 인기 상품 목록
       $result = mysql_query("select * from product where name like '%물고기%' order by hit desc", $con);
		break;
}
$total=mysql_num_rows($result);	
		...
		if (!$total){
			...
			if($mode==0){
			echo("
			<td align=right><a href='event2.php?mode=0&#bi' id='rr' style='font-weight:bold;' >상품명</a>
			| <a href='event2.php?mode=1&#bi' id='rr'>낮은 가격</a>
		|<a href='event2.php?mode=2&#bi'  id='rr'>높은 가격</a> | <a href='event2.php?mode=3&#bi'  id='rr'>인기순</a></td></tr></table>
		");
			}
			else if($mode==1){
			echo("
			<td align=right><a href='event2.php?mode=0&#bi' id='rr'  >상품명</a>
			| <a href='event2.php?mode=1&#bi' id='rr' style='font-weight:bold;'>낮은 가격</a>
		|<a href='event2.php?mode=2&#bi'  id='rr'>높은 가격</a> | <a href='event2.php?mode=3&#bi'  id='rr'>인기순</a></td></tr></table>
		");
			}
					else if($mode==2){
			...
			<td align=right><a href='event2.php?mode=0&#bi' id='rr'  >상품명</a>
			| <a href='event2.php?mode=1&#bi' id='rr' >낮은 가격</a>
		|<a href='event2.php?mode=2&#bi'  id='rr' style='font-weight:bold;'>높은 가격</a> | <a href='event2.php?mode=3&#bi'  id='rr'>인기순</a></td></tr></table>
		");
			}
						else if($mode==3){
			echo("
			<td align=right><a href='event2.php?mode=0&#bi' id='rr'  >상품명</a>
			| <a href='event2.php?mode=1&#bi' id='rr' >낮은 가격</a>
		|<a href='event2.php?mode=2&#bi'  id='rr'>높은 가격</a> | <a href='event2.php?mode=3&#bi'  style='font-weight:bold;'  id='rr'>인기순</a></td></tr></table>
		");
			}
						else{
			echo("
...
$price3=$price2+($price2*0.1);
	...
	if ($cpage==1)     //현재 페이지가 1인 경우 맨앞블록 비활성화
		...
		echo("<a href=event2.php?cblock=1&cpage=1&class=$class&mode=$mode><i class='fas fa-angle-double-left' id='gright'></i></a> ");
	if ($pblock > 0)        // 이전 블록이 존재하면 [이전블록] 버튼을 활성화
		echo ("<a href=event2.php?cblock=$pblock&cpage=$pstartpage&class=$class&mode=$mode><i class='fa-solid fa-chevron-left' id='gright'></i></a> ");
	...
	while($i < $nstartpage):
	   ...
	   if ($cpage==$i){
	   echo ("<a href=event2.php?cblock=$cblock&cpage=$i&class=$class&mode=$mode><button style='font-size:13; border-radius:5px; background-color:black; color:white; padding:10px;'>$i</button></a>");
	   $i = $i + 1;}
	   else{
		   echo ("<a href=event2.php?cblock=$cblock&cpage=$i&class=$class&mode=$mode><button class='c'>$i</button></a>");
	   $i = $i + 1;} 
	...
	if ($nstartpage <= $totalpage)   
		echo ("<a href=event2.php?board=testboard&cblock=$nblock&cpage=$nstartpage&class=0><i class='fa-solid fa-chevron-right' id='gright'></i></a> ");
	if ($totalpage<=$cpage)
		echo("");
	else
		
	echo ("<a href=event2.php?board=testboard&cpage=$totalpage&cblock=$ff&class=0><i class='fas fa-angle-double-right' id='gright'></i></a> ");//모르겠다수박
