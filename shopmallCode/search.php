...
//if (!isset($class)) $class=0;
   ...
   case 0:        // 초기화면에 출력할 인기 상품 목록
       $result = mysql_query("select * from product where name like '%$key%' order by name", $con);
		break;
   case 1:        // 초기화면에 출력할 인기 상품 목록
       $result = mysql_query("select * from product where name like '%$key%' order by price2", $con);
		break;
...
$total=mysql_num_rows($result);	
 ...
 include "top.html"; 
	...
	echo("	<center><table border=0 width=1000 style='margin-top:250px;'><tr>");
		...
		echo("<td align=right colspan=5  ><a href='search.php?mode=0&key=$key' id='rr'> 상품명 </a>| <a href='search.php?mode=1&key=$key'  id='rr'>낮은 가격</a>
		|<a href=search.php?mode=2&key=$key  id='rr'>높은 가격</a> | <a href=search.php?mode=3&key=$key  id='rr'>인기순</a></td></tr>");
				...
				while ($counter < $total && $counter < 15 && $subcounter < $pagesize) :
					...
					break;
						...
						echo ("<tr><td colspan=5></td></tr><tr height=200>");
						}
					}
...
$reviewnumre=mysql_query("select * from testboard where code='$code'",$con);
	...
	endwhile;
	echo("</tr></table>");
	echo ("<table border=0 width=700 align=center style='margin-top:100px;'>
		  <tr><td align=center>");
	...
	if ($cpage==1)     //현재 페이지가 1인 경우 맨앞블록 비활성화
		...
		echo("<a href='search.php?cblock=1&cpage=1&class=$class&mode=$mode&key=$key'><i class='fas fa-angle-double-left' id='gright'></i></a> ");
	if ($pblock > 0)        // 이전 블록이 존재하면 [이전블록] 버튼을 활성화
		echo ("<a href=search.php?cblock=$pblock&cpage=$pstartpage&class=$class&mode=$mode&key=$key><i class='fa-solid fa-chevron-left' id='gright'></i></a> ");
	...
	while($i < $nstartpage):
	   ...
	   if ($cpage==$i){
	   echo ("<a href='search.php?cblock=$cblock&cpage=$i&class=$class&mode=$mode&key=$key'><button style='font-size:13; border-radius:5px; background-color:black; color:white; padding:10px;'>$i</button></a>");
	   $i = $i + 1;}
	   else{
		   echo ("<a href='search.php?cblock=$cblock&cpage=$i&class=$class&mode=$mode&key=$key'><button class='c'>$i</button></a>");
	   $i = $i + 1;} 
	...
	if ($nstartpage <= $totalpage)   
		echo ("<a href='search..php?board=testboard&cblock=$nblock&cpage=$nstartpage&class=$class&mode=$mode&key=$key'><i class='fa-solid fa-chevron-right' id='gright'></i></a> ");
	if ($totalpage<=$cpage)
		echo("");
	else
		
	echo ("<a href='search.php?board=testboard&cpage=$totalpage&cblock=$ff&class=$class&mode=$mode&key=$key'><i class='fas fa-angle-double-right' id='gright'></i></a> ");//모르겠다수박
