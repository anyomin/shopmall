...
.aa:hover{
 ...
 <?
  include ("top.html");
		...
		$result = mysql_query("select * from testboard where writer='$UserID'", $con);
		$total = mysql_num_rows($result);
	
		echo("
		<table border=0 width=1000 align=center  style='border-top:1px solid;margin-top:180px;' >
	<tr height=35 class='tt' ><td align=center width=50><b>번호</b></td>
		...
		if (!$total){
	echo("
		<tr><td colspan=5 align=center>아직 후기글이 없습니다.</td></tr>
	");
} else {
		...
		if   ($space>0) {
			...
			if ($filename):
				echo("
					...
					<td align=left>$t<a  href='myreviewcontent.php?board=testboard&id=$id&cblock=$cblock&ccpage=$cpage&pcode=$code&id=$id&ordernum=$ordernum' class='aa'>$topic</a>&nbsp;<i  class='fa-solid fa-paperclip'></i></td>
				...
				echo("
					...
					<td align=center width=200><a href=# onclick=\"window.open('./photo/$userfile', '_new', 'width=200, height=200')\"><img src='./photo/$userfile' width=170 style='height:170px;margin:10px;' border=1></a></td>
					<td align=center>$writer</td>
					<td align=left>$t<a href='myreviewcontent.php?board=testboard&id=$id&cblock=$cblock&ccpage=$cpage&pcode=$code&ordernum=$ordernum' class='aa'>$topic&nbsp;</a></td>
					<td align=center>$wdate</td><td align=center>$hit</td>
					<td align=center><i class='fa-solid fa-star' style='color:#F6D505;'></i>X<a>$star</a></td>
					</tr>
					");
	...
	if ($cpage==1)     //현재 페이지가 1인 경우 맨앞블록 비활성화
		...
		echo("<a href=myreview.php?board=testboard&cblock=1&cpage=1><i class='fas fa-angle-double-left' id='gright'></i></a> ");
	if ($pblock > 0)        // 이전 블록이 존재하면 [이전블록] 버튼을 활성화
		echo ("<a href=myreview.php?board=testboard&cblock=$pblock&cpage=$pstartpage><i class='fa-solid fa-chevron-left' id='gright'></i></a> ");
	...
	while($i < $nstartpage):
	   ...
	   if ($cpage==$i){
	   echo ("<a href=myreview.php?board=testboard&cblock=$cblock&cpage=$i><button style='font-weight:bold;color:balck;background-color:white;font-size:15px;padding:12px;border:none;' >$i</button></a>");
	   $i = $i + 1;}
	   else{
		   echo ("<a href=myreview.php?board=testboard&cblock=$cblock&cpage=$i><button  id='butt'>$i</button></a>");
	   $i = $i + 1;} 
	...
	if ($nstartpage <= $totalpage)   
		echo ("<a href=myreview.php?board=testboard&cblock=$nblock&cpage=$nstartpage><i class='fa-solid fa-chevron-right' id='gright'></i></a> ");
	if ($totalpage<=$cpage)
		echo("");
	else
		
	echo ("<a href=myreview.php?board=testboard&cpage=$totalpage&cblock=$ff><i class='fas fa-angle-double-right' id='gright'></i></a> ");//모르겠다수박

	echo ("</td></tr><tr><td align=right colspan=2><a href='mypage.php' >Mypage</a></td></tr></table>");
}
